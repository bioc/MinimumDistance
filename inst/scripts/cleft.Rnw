...
ped <- read.csv("~/Projects/BeatyExperimentData/inst/extdata/may_peds.csv", sep=";",
		stringsAsFactors=FALSE)
## functions in CleftExperimentDAta needed for Cleft study
...
if(exists("pedigreeIndex")) pedigree <- pedigree[pedigreeIndex, ]
trioSetListff <- TrioSetListLD(path=path, fnames=allNames(pedigree), ext=".txt",
			       samplesheet=samplesheet, ##optional
			       row.names=samplesheet$Sample.Name, ## must provide if samplesheeet is not missing
			       pedigree=pedigree,
...
<<save.lrr.segs>>=
save(lrr.segs, file=file.path(rangesdir, "lrr.segs.rda"))
@
...
<<save.md.segs>>=
save(md.segs, file=file.path(rangesdir, "md.segs.rda"))
@
...
<<save.map.segs>>=
save(map.segs, file="/local_data/r00/beaty/ranges/map.segs.rda")
@
