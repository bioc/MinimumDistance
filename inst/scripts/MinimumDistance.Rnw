...
trioFactor <- matrix(seq_len(nrow(pedigreeExample)), nrow(pedigreeExample), 3, byrow=FALSE)
...
Segment the minimum distance and return a \Robject{RangedDataCBS}
object.  For illustration, we fit only the first 10 chromosomes

<<segmentMinimumDistance>>=
md.segs <- xsegment(object=trioSetList[1:10], segment.mindist=TRUE, verbose=TRUE)
md.segs
@
...
<<segmentLRR>>=
lrr.segs <- xsegment(object=trioSetList[1:10], segment.mindist=FALSE,
...
rd <- computeBayesFactor(object=trioSetList,
			 ranges=md.segs2,
			 tau=tau, log.pi=log.pi)
rd <- MinimumDistance:::pruneByFactor(rd,f=rd$argmax,verbose=TRUE)
