...
pedigree <- as(pedigreeExample, "Pedigree")
trio.ann <- new("TrioAnnotation", sampleSheet=sample.sheet, pedigree=pedigree)
validObject(trio.ann)
sampleNames(trio.ann)
data(logRArray)
data(bafArray)

## replace phenoDataArray with a coercion from TrioAnnotation to an array
## The coercion should take place in the 'TrioSetList' constructor
##pData <- as(trio.ann, "array")
trioSetList <- TrioSetList(trioAnnotation=trio.ann,
			   logR=logR,
			   baf=baf,
			   cdfname="human610quadv1bCrlmm")
##trioSetList <- constructTrioSetList(pData=pData,
