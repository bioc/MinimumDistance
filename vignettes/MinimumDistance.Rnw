...
\Robject{TrioSetList} class has a slot for pedigree information defining
...
object of class \Robject{Pedigree} and \Robject{SampleSheet} that contain
this information.
...
the BeadStudio output. These matrices contain 25 markers for each
chromosome as the intent is to illustrate the steps required for
initializing a data structure of the \Robject{TrioSetList} class. (In
order to illustrate some of the smoothing functions downstream, we
will use the \Robject{trioSetListExample} data provided with the
package.  The \Robject{trioSetListExample} contains many more markers,
but only for two autosomes). A \Rclass{TrioSetList} object can be
created using the helper function \Rfunction{TrioSetList}.
...
arrays using the information stored in the \Robject{pedigree} object.
...
In order to make the size of this package manageable, the
...
illustrating the steps for creating the required data structures, it
is not useful for detetecting de novo copy number alterations.  To
illustrate the smoothing steps, we load an object that contains all
the markers for 2 chromosomes (7 and 22) for 2 HapMap trios. A few
summary statistics for this object are displayed below.
