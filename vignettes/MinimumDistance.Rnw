...
particularly useful for visual inspection of the copy number calls.
...
Before we read in the data, we must specify the samples belonging to a
trio and indicate the trio membership as father (F), mother (M), and
offspring (O):
...
Note that if we had multiple trios, the arguments to F, M, and O are
vectors. The sample identifiers for father and mother can be
duplicated, but the offspring sample identifiers must be unique. We
...
We are now ready to read in the files containing the log R ratios and
B allele frequencies.  The function \Rfunction{callDenovoSegments}
...
<<callDenovoSegments>>=
trace(calculateMindist, browser, signature="array")
tmp <- calculateMindist(lrr(trioSetList))
pos <- position(trioSetList)
trace(position, browser, signature="TrioSetList")
