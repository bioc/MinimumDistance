...
We use a combination of lattice and grid to visualize the marker-level
summaries and the copy number inference for the trio.


<<plotDenovo1, fig=TRUE, width=8, height=6>>=
library(grid)
g <- denovoHemizygous(md_g)
vps <- MinimumDistance:::pedigreeViewports()
param <- HmmTrellisParam()
p <- plotDenovo(me, g[1], param)
...
denovo segment.  In the following code-chunk, we reduce the denovo
hemizygous deletions, update the posterior probabilities for the MAP
estimates, and visualize the result.

<<plotDenovo2, fig=TRUE, width=8, height=6>>=
g2 <- reduce(denovoHemizygous(md_g), min.gapwidth=500e3)
post <- MAP2(me, g2, params)
g2 <- denovoHemizygous(post)
p <- plotDenovo(me, g2, param)
