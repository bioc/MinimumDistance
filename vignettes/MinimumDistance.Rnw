...
class \Robject{RangedDataHMM}. The state symbols are the integer copy
number inferred for the father, mother and offspring with an offset of
1. For example, the symbol for state '321' would indicate two DNA
copies in father, 1 copy in the mother, and 0 copies in the offspring.
In the above example, de novo copy number alterations were not
detected and the state '333' indicates that the father, mother, and
offspring appear to be diploid for the interval beginning at
...
more manageable and to facilitate parallelization (not yet
implemented), we organize the low-level summaries by chromosome using
the \Rclass{TrioSetList} class. The \Rclass{TrioSetList} class has a
slot for storing an object of class \Robject{Pedigree} (discussed
previously), as well as slots for storing sample-level covariates on
the offspring (\Robject{phenoData}), mother
(\Robject{motherPhenoData}), and the father
(\Robject{fatherPhenoData}).  Often technical, experimental, and
phenotypic covariates for the samples in an experiment are stored in a
single tab-delimited file in which columns are covariates and rows are
samples.  Such a spreadsheed can be read into \R{} as a
...
\Robject{assayDataList} slot of the \Rclass{TrioSetList}
class. Accessors \Rfunction{lrr} and \Rfunction{baf} are available for
extracting the list of arrays for the log R ratios and B allele
frequencies.
...
frame <- 100e3
figs <- MinimumDistance:::xyplotTrioLrrBaf(rd=rd.denovoDel,
					   object=trioSet,
					   frame=frame,
...
\subsection{Outline for Large Data / Parallelization}

For large data sets, we have developed a pipeline that uses the
\Rpackage{ff} package for large data support and the package
\Rpackage{snow} for parallelization.  Currently, the central
requirement for this approach is to parse the log R ratios and B
allele frequencies for each sample to a separate file with the format
roughly as in 'F.txt' (a small included with this package that we read
in the previous section). The following (unevaluated) code chunks
provide a general sense of the set-up and workflow. First, we enable
large data support and register a backend for parallelization.

<<lds, eval=FALSE>>=
library2(ff)
library2(snow)
library2(doSNOW)
ffdir <- "<PATH_FOR_FF_FILES>"
ldPath(ff)
cl <- makeCluster(22, "SOCK")
registerDoSNOW(cl)
@

Next, we initialize a \Rclass{TrioSetList} object with the function
\Rfunction{TrioSetListLD}.  The structure of this object is the same
as in the previous sections, but the assay data (log R ratios and B
allele frequencies) are stored in objects with the class
\Rclass{ff\_array}.  In particular, this means that the assay data
elements contain pointers to data stored on disk.  In the following
code chunk, we call the \Rfunction{TrioSetListLD} function by
specifying the path to the plain text files containing the log R
ratios and B allele frequencies for each sample and the
\Robject{pedigreeData} object.

<<trioSetListLD, eval=FALSE>>=
trioSetListff <- TrioSetListLD(path=path,
			       fnames=allNames(pedigreeData),
			       ext=".txt",
			       pedigree=pedigreeData,
			       annotationPkg="human610quadv1bCrlmm")
@

The remaining steps are the same as in the previous section:

<<segmentationPosteriorCalling,eval=FALSE>>=
mdlist <- calculateMindist(lrr(trioSetListff), outdir=ldPath())
lrr.segs <- segment2(trioSetListff)
md.segs <- segment2(trioSetListff, md=mdlist)
map.segs <- computeBayesFactor(trioSetListff, ranges=md.segs2)
@

We remark that the steps in the previous code chunk benefit from
parallelization and do not require enabling large data support with
the \Rpackage{ff} package. However, with large data support enabled we
have processed 1000 trios (3000 samples) on 22 worker nodes using a
total of 100 Gb RAM (4-5 Gb per worker) in less than 24 hours.
