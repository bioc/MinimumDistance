...
backend so that warning messages do not appear during processing.
This is accomplished using the function \Rfunction{registerDoSEQ} in
the \Rpackage{foreach} package.
