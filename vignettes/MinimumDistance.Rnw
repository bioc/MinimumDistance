...
rd <- rd.denovoDel[1, ]
CHR <- match(chromosome(rd), chromosome(trioSetList))
...
\begin{figure}[t]
  ...
  \caption{\label{fig:trioplot} The log R ratios (black) and B allele
    frequencies (blue) for the father, mother, and offspring, as well
    as the minimum distance (bottom).  The solid vertical lines in the
    bottom panel indicate the start and stop positions of an inferred
    de novo hemizygous deletion in the offspring (state '332').  }
\end{figure}
