...
Sections \ref{option2} and \ref{visualization}, respectively.  Where
...
Accessors for sample-level covariates:
...
Useful accessors for \Rclass{RangedDataCBS} objects include
<<RangedDataCBSaccessors>>=
sampleNames(md.segs)[1:4]
coverage2(md.segs)[1:4]
chromosome(md.segs)[1:4]
mean(md.segs)[1:4]
@

An \textit{ad-hoc} approach to increase the chance that a given
segment obtained from the segmentation of the minimum distance is
homogeneous with respect to the offspring copy number is to edit the
start and stop position of segments with a nonzero mean minimum
distance.  The approach currently implemented is to define a new start
and stop position if a breakpoint of the segmented log R ratios for
the offspring occurs in the interval. To illustrate, the following
...
Note that the above method could be applied to a single genomic
interal.  For example,
